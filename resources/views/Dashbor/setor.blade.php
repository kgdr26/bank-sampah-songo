        ...
        <div class="row">
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            <h4 class="card-title">Data Anggota Setor Sampah</h4>
                        </div>
                    </div>
                    <div class="card-body">
                        <form id="addsetor">
                            <div class="form-group">
                                <label class="form-label" for="msa_name">Nama Anggota</label>
                                <select class="form-select select2" name="tts_msa_id" id="tts_msa_id">
                                    <option value="">Choose...</option>
                                    @foreach ($list_anggota as $key => $val)
                                        <option value="{{$val->msa_id}}">{{$val->msa_name}}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class="form-group">
                                <label class="form-label" for="tts_setor_date">Tanggal Setor</label>
                                <div class="form-group input-group">
                                    <input type="text" class="form-control datepicker" aria-label="Tanggal Setor" aria-describedby="tts_setor_date" id="tts_setor_date" name="tts_setor_date">
                                    <span class="input-group-text" id="date_setor">
                            ...
                            <div class="form-group">
                                <label class="form-label" for="tts_mcs_id">Categori Sampah</label>
                                <select class="form-select select2" name="tts_mcs_id" id="tts_mcs_id">
                                    <option value="">Choose...</option>
                                    @foreach ($list_sampah as $key => $val)
                                        <option value="{{$val->mcs_id}}">{{$val->mcs_code}} : {{$val->mcs_name}}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class="form-group">
                                <label class="form-label" for="tts_berat_sampah">Total Berat</label>
                                <input type="tts_berat_sampah" class="form-control" id="tts_berat_sampah" name="tts_berat_sampah">
                            </div>

                            <div class="d-flex justify-content-end">
                                <button type="button" class="btn btn-info me-2" data-name="show_data">Show Data</button>
                                <button type="button" class="btn btn-primary" data-name="save_setor">Add</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div class="col-xl-8 col-md-12 col-sm-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            <h4 class="card-title">List Data Setor Sampah</h4>
                        </div>
                    </div>
                    <div class="card-body">
